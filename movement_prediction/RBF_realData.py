...
from functions import comb_dataset_RealData
...
def err_on_basis_mean_traj_err1 (N, time_steps):
    ...
    dataset = comb_dataset_RealData(time_steps)[:3]  # get dataset
...
# err_left = []
# N = []
# for n in range(2, 15): # 0 = right,  1 = straight, 2 = left
...
# axes[2].set_title('Error of going Left')
#
#
# plt.xlabel('# of basis functions')
# axes[1].set_ylabel('Reconstructed trajectory error')
# axes[0].plot(N, err_right, 'bo')
# axes[1].plot(N, err_straight, 'mo')
# axes[2].plot(N, err_left, 'go')
#
...
axes[1].plot(means_right[:, 0], color="black")
axes[2].plot(means_right[:, 1], color="black")
axes[3].plot(means_right[:, 0], means_right[:, 1], color="black", label=labels['mean'])

axes[1].plot(means_straight[:, 0], color="black")
axes[2].plot(means_straight[:, 1], color="black")
axes[3].plot(means_straight[:, 0], means_straight[:, 1], color="black")

axes[1].plot(means_left[:, 0], color="black")
axes[2].plot(means_left[:, 1], color="black")
axes[3].plot(means_left[:, 0], means_left[:, 1], color="black")
...
for i in range(time_steps):
    ...
    circle_right_x = plt.Circle((i, means_right[i][0]), radius = np.linalg.norm(std_right[i]))
    circle_right_y = plt.Circle((i, means_right[i][1]), radius = np.linalg.norm(std_right[i]))
    circle_straight_x = plt.Circle((i, means_straight[i][0]), radius = np.linalg.norm(std_straight[i]))
    circle_straight_y = plt.Circle((i, means_straight[i][1]), radius = np.linalg.norm(std_straight[i]))
    circle_left_x = plt.Circle((i, means_left[i][0]), radius = np.linalg.norm(std_left[i]))
    circle_left_y = plt.Circle((i, means_left[i][1]), radius = np.linalg.norm(std_left[i]))
...
random_trajectory_count = 4 # randomly selets chosen number of trajectories
...
for i in range(0,random_trajectory_count):
    ...
    plt.legend(loc='center right', bbox_to_anchor=(3, 1.5))
plt.show()