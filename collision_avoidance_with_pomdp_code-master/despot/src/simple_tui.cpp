...
void SimpleTUI::InitializeEvaluator(Evaluator *&simulator,
    ...
    /* Create the Evaluator for showing stuff on the terminal. */
...
void SimpleTUI::DisplayParameters(option::Option *options, DSPOMDP *model) {
    ...
    string lbtype = options[E_LBTYPE] ? options[E_LBTYPE].arg : "DEFAULT";
    string ubtype = options[E_UBTYPE] ? options[E_UBTYPE].arg : "DEFAULT";
    logi << "Model = " << typeid(*model).name() << endl
...
bool got_goal = false;
bool got_start_car = false;
bool got_start_ped = false;
bool got_pedestrian_goals = false;
bool got_path = false;

void cb_car_goal(const geometry_msgs::PoseStamped::ConstPtr &msg)
{
    if (!got_goal) {
        Car::set_goal(*msg);
        got_goal = true;
        default_out << "Got Car Goal: " << msg->pose.position.x << ", " << msg->pose.position.y << std::endl;
    }
}

void cb_car_start_position(const nav_msgs::OdometryPtr &msg)
{
    if (!got_start_car) {
        got_start_car = true;
        Car::set_car_start(msg->pose);
    }
}

void cb_ped_start_position(const nav_msgs::OdometryPtr &msg)
{
        Car::set_ped_start(msg->pose.pose.position.x, msg->pose.pose.position.y);
        got_start_ped = true;
        got_goal = false;
}

void cb_pedestrian_goals(const car_model::GoalsConstPtr &msg)
{
    if (!got_pedestrian_goals) {
        Car::set_pedestrian_goals(msg->goals);
        got_pedestrian_goals = true;
    }
}

void cb_get_path(const nav_msgs::PathConstPtr &msg)
{
    if (!got_path)
    {
        Car::set_path(msg->poses);
        got_path = true;
    }
}
...
void SimpleTUI::RunEvaluator(DSPOMDP *model, Evaluator *simulator,
                             ...
                             int start_run) {
    default_out << "\033[1mWaiting for goal ...\033[0m" << endl;

    ros::NodeHandle nodeHandle("");

    // TODO: Look for a better solution here to publish goal and get the position of everything :(

    ros::Subscriber sub_goal = nodeHandle.subscribe("/move_base_simple/goal", 1, cb_car_goal);
    ros::Subscriber sub_car_start = nodeHandle.subscribe("/odom", 1, cb_car_start_position);
    ros::Subscriber sub_ped_goals = nodeHandle.subscribe("/pedestrian_goals", 1, cb_pedestrian_goals);
    ros::Subscriber sub_ped_start = nodeHandle.subscribe("/pedestrian_position", 1, cb_ped_start_position);
    ros::Subscriber sub_path = nodeHandle.subscribe("/move_base/TebLocalPlannerROS/global_plan", 1, cb_get_path);

    ros::Publisher goal_publisher = nodeHandle.advertise<geometry_msgs::PoseStamped>("/move_base_simple/goal", 1000);
    ros::Publisher start_planner_publisher = nodeHandle.advertise<std_msgs::Bool>("/start_pedestrian_planner", 1000);
    ros::Publisher replan_publisher = nodeHandle.advertise<std_msgs::Bool>("/freeze_obstacle_position", 1);


    geometry_msgs::PoseStamped GOAL;
    GOAL.header.frame_id = "map";
    GOAL.pose.position.x = 12.0;
    GOAL.pose.position.y = 35.0;
    GOAL.pose.orientation.w = 1.0;
    std::cout << "Setting up goal... " << std::endl;

    while (!got_start_ped || !got_pedestrian_goals)
    {
        ros::spinOnce();
    }
...
//    }
    ...
    sub_goal.shutdown();
    sub_ped_start.shutdown();
    sub_car_start.shutdown();
    sub_ped_goals.shutdown();
    sub_path.shutdown();


    auto start = std_msgs::Bool();
    start.data = '1';
    start_planner_publisher.publish(start);
    ...
    for (int round = start_run; round < start_run + num_runs; round++) {
        ...
        for (int i = 0; i < Globals::config.sim_len; i++) {
            ...
            if (step_end_t - step_start_t < 0.1) {
                std::cout << "\033[1;31m SOMETHING IS WRONG OH NOOOOO TOO FAAASSTTT\033[0m\n";
            }

            cout << "[main] Time for step: actual / allocated = "
                 << (step_end_t - step_start_t) << " / " << EvalLog::allocated_time
                 << endl;
            simulator->UpdateTimePerMove(step_end_t - step_start_t);
            cout << "[main] Time per move set to " << Globals::config.time_per_move
                 << endl;
            cout << "[main] Plan time ratio set to " << EvalLog::plan_time_ratio
                 << endl;

            // Send trigger to replan local path
            if (i % 5 == 0) {
                std::cout << "\033[32;1m Replanning at time step \033[0m" << i << std::endl;
                std_msgs::Bool b;
                b.data = static_cast<unsigned char>(true);
                replan_publisher.publish(b);
            }
        }
