...
vector<State*> Belief::Sample(int num, vector<State*> particles, const DSPOMDP* model) {
    double unit = 1.0 / num;
...
//	default_out << "[Belief::Sample] Sampled " << sample.size() << " particles"
//		<< endl;
//	for (int i = 0; i < sample.size(); i++) {
//		default_out << " " << i << " = " << *sample[i] << endl;
//	}
...
ParticleBelief::ParticleBelief(vector<State*> particles, const DSPOMDP* model,
    ...
    if (fabs(State::Weight(particles) - 1.0) > 1e-2) {
        default_out << "[ParticleBelief::ParticleBelief] Particle weights sum to " << State::Weight(particles) << " instead of 1" << endl;
        exit(1);
    }
...
//	 default_out << "Number of particles in initial belief: " << particles_.size() << endl;
...
void ParticleBelief::Update(int action, OBS_TYPE obs) {
    ...
    default_out << "[ParticleBelief::Update] " << updated.size()
        << " particles survived among " << particles_.size() << endl;
//    default_out << updated[0]->weight << endl;
//    default_out << updated[28]->weight << endl;
    particles_ = updated;
