...
Policy::Policy(const DSPOMDP* model, ParticleLowerBound* particle_lower_bound, Belief* belief) :
...
ValuedAction Policy::Value(const vector<State*>& particles,
    ...
    FileWriter::Write("DEBUG", "[DESPOT::Value] About to call recursive value function...");
...
ValuedAction Policy::RecursiveValue(const vector<State*>& particles,
    ...
    if (streams.Exhausted()
        ...
        OBS_TYPE obs;
        double reward;
        FileWriter::Write("DEBUG", "[Policy::RecursiveValue] About to go through every particle and execute STEP function...");
