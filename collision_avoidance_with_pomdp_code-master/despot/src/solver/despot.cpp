...
#include "../../examples/cpp_models/car/include/pomdp_model/CarState.h"
...
VNode* DESPOT::Trial(VNode* root, RandomStreams& streams,
    ...
    do {
        ...
        if (cur->IsLeaf()) {
            double start = clock();
            FileWriter::Write("DEBUG", "[DESPOT::Trial] At leaf node, going to Expand it...");
...
VNode* DESPOT::ConstructTree(vector<State*>& particles, RandomStreams& streams,
    ...
    FileWriter::Write("DEBUG", "[DESPOT::ConstructTree] Making root vnode!");
    VNode* root = new VNode(particles);
    FileWriter::Write("DEBUG", "[DESPOT::ConstructTree] Done making root vnode!");
    ...
    FileWriter::Write("DEBUG", "[DESPOT::ConstructTree] Going into do-while loop...");

    double used_time = 0;
    int num_trials = 0;
    do {
        double start = clock();
        FileWriter::Write("DEBUG", "[DESPOT::ConstructTree] About to execute Trial...");
        VNode* cur = Trial(root, streams, lower_bound, upper_bound, model, history, statistics);
        used_time += double(clock() - start) / CLOCKS_PER_SEC;
        FileWriter::Write("DEBUG", "[DESPOT::ConstructTree] Finished Trial here!");
...
void DESPOT::InitLowerBound(VNode* vnode, ScenarioLowerBound* lower_bound,
    ...
    FileWriter::Write("DEBUG", "[DESPOT::InitLowerBound] Init lower bound with VALUE function...");
...
ValuedAction DESPOT::Search() {
    ...
    vector<State*> particles = belief_->Sample(Globals::config.num_scenarios);
    FileWriter::WriteArray("DEBUG", particles);
    logd << "[DESPOT::Search] Time for sampling " << particles.size()
    ...
    FileWriter::Write("DEBUG", "[DESPOT::Search] Constructing Tree....");

    root_ = ConstructTree(particles, streams, lower_bound_, upper_bound_,
        model_, history_, Globals::config.time_per_move, &statistics_);
    logd << "[DESPOT::Search] Time for tree construction: "
        << (get_time_second() - start) << "s" << endl;

    FileWriter::Write("DEBUG", "[DESPOT::Search] Back from ConstructTree!");


    start = get_time_second();
    root_->Free(*model_);
    logd << "[DESPOT::Search] Time for freeing particles in search tree: "
        << (get_time_second() - start) << "s" << endl;
    ...
    logd << "[DESPOT::Search] Time for deleting tree: "
        << (get_time_second() - start) << "s" << endl;
    logd << "[DESPOT::Search] Search statistics:" << endl << statistics_
        << endl;
...
double DESPOT::CheckDESPOT(const VNode* vnode, double regularized_value) {
    ...
    FileWriter::Write("DEBUG", "[DESPOT::CheckDESPOT] Checking DESPOT....");
    ...
    do {
        double start = clock();
        FileWriter::Write("DEBUG", "[DESPOT::CheckDESPOT] About to execute Trial...");
...
ValuedAction DESPOT::OptimalAction(VNode* vnode) {
    ...
    for (int action = 0; action < vnode->children().size(); action++) {
        ...
        FileWriter::Write("DEBUG", "DESPOT::OptimalAction: Action " + std::to_string(action) + ", Weight " + std::to_string(qnode->Weight()));
...
void DESPOT::Expand(VNode* vnode,
    ...
    FileWriter::Write("DEBUG", "[DESPOT::Expand] Going for every action now");
    ...
    for (int action = 0; action < model->NumActions(); action++) {
        ...
        FileWriter::Write("DEBUG", "[DESPOT::Expand] Action " + std::to_string(action));

        Expand(qnode, lower_bound, upper_bound, model, streams, history);
    }
    logd << "* Expansion complete!" << endl;
    FileWriter::Write("DEBUG", "[DESPOT::Expand] Finished Expand! ----------------------------------------------------------");
...
void DESPOT::Expand(QNode* qnode, ScenarioLowerBound* lb,
    ...
    FileWriter::Write("DEBUG", "[DESPOT::Expand] Inisde real Expand Function!");
    ...
    for (int i = 0; i < particles.size(); i++) {
        ...
        // TODO: [Marker]
        logd << " ##############################################\n Original: " << *particle << endl;

        State* copy = model->Copy(particle);
        logd << " Before step: " << *copy << endl;
        CarState * cs = static_cast<CarState*>(copy);
        FileWriter::Write("DEBUG", "[DESPOT::Expand] Iteration " + std::to_string(i+1) +" with current state " + cs->text());

        bool terminal = model->Step(*copy, streams.Entry(copy->scenario_id),
            qnode->edge(), reward, obs);

        FileWriter::Write("DEBUG", "[DESPOT::Expand] After Step Function: " + cs->text());
    ...
    FileWriter::Write("DEBUG", "[DESPOT::Expand] FINISHED LOOPING #####################################################");
...
ValuedAction DESPOT::Evaluate(VNode* root, vector<State*>& particles,
    ...
    return {OptimalAction(root).action, value / particles.size()};
}

void DESPOT::belief(Belief* b) {
    logd << "[DESPOT::belief] Start: Set initial belief." << endl;
    belief_ = b;
    history_.Truncate(0);

    lower_bound_->belief(b); // needed for POMCPScenarioLowerBound
    logd << "[DESPOT::belief] End: Set initial belief." << endl;
}
...
void DESPOT::Update(int action, OBS_TYPE obs) {
    ...
    logd << "[Solver::Update] Updated belief, history and root with action "
